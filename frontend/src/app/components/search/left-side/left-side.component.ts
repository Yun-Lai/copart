...
export class LeftSideComponent implements OnInit {
  ...
  getListData() {
    ...
    this.api.getSearchListYearLocation(this.params).then(result => {
      ...
      this.sale_dates = result.sale_dates;
    }).catch(error => {
      console.log(error);
    });
    this.api.getSearchListVehicleDamage(this.params).then(result => {
      this.vehicle_types = result.vehicle_types;
      this.damages = result.damages;
    ...
    this.api.getSearchListTransmissionCylinder(this.params).then(result => {
      ...
      this.cylinders = result.cylinders;
    }).catch(error => {
      console.log(error);
    });
  }
  ...
  addParams(key, value: string) {
    if (key === 'featured') {
      ...
      if (this.applied_filter_odometers.indexOf(value) >= 0){
        let filter = this.applied_filter_odometers.filter(item => item !== value);
        this.changeFilters.emit({"key": key, "value": filter});
      } else {
        let filter = Object.assign(this.applied_filter_odometers);
        filter.push(value);
        this.changeFilters.emit({"key": key, "value": filter});
      }
    } else if (key === 'locations') {
      if (this.applied_filter_locations.indexOf(value) >= 0){
        let filter = this.applied_filter_locations.filter(item => item !== value);
        this.changeFilters.emit({"key": key, "value": filter});
      } else {
        let filter = Object.assign(this.applied_filter_locations);
        filter.push(value);
        this.changeFilters.emit({"key": key, "value": filter});
      }
    } else if (key === 'sale_dates') {
      if (this.applied_filter_sale_dates.indexOf(value) >= 0){
        let filter = this.applied_filter_sale_dates.filter(item => item !== value);
        this.changeFilters.emit({"key": key, "value": filter});
      } else {
        let filter = Object.assign(this.applied_filter_sale_dates);
        filter.push(value);
        this.changeFilters.emit({"key": key, "value": filter});
      }
    } else if (key === 'doctypes') {
      if (this.applied_filter_doctypes.indexOf(value) >= 0){
        let filter = this.applied_filter_doctypes.filter(item => item !== value);
        this.changeFilters.emit({"key": key, "value": filter});
      } else {
        let filter = Object.assign(this.applied_filter_doctypes);
        filter.push(value);
        this.changeFilters.emit({"key": key, "value": filter});
      }
    } else if (key === 'vehicle_types') {
      if (this.applied_filter_vehicle_types.indexOf(value) >= 0){
        let filter = this.applied_filter_vehicle_types.filter(item => item !== value);
        this.changeFilters.emit({"key": key, "value": filter});
      } else {
        let filter = Object.assign(this.applied_filter_vehicle_types);
        filter.push(value);
        this.changeFilters.emit({"key": key, "value": filter});
      }
    } else if (key === 'damages') {
      if (this.applied_filter_damages.indexOf(value) >= 0){
        let filter = this.applied_filter_damages.filter(item => item !== value);
        this.changeFilters.emit({"key": key, "value": filter});
      } else {
        let filter = Object.assign(this.applied_filter_damages);
        filter.push(value);
        this.changeFilters.emit({"key": key, "value": filter});
      }
      ...
      if (this.applied_filter_cylinders.indexOf(value) >= 0){
        let filter = this.applied_filter_cylinders.filter(item => item !== value);
        this.changeFilters.emit({"key": key, "value": filter});
      } else {
        let filter = Object.assign(this.applied_filter_cylinders);
        filter.push(value);
        this.changeFilters.emit({"key": key, "value": filter});
      }
    } else if (key === 'source') {
