...
@task(
    name="product.tasks.generate_cache",
    ignore_result=True,
    time_limit=50400,
    queue='high',
    option={'queue': 'high'}
)
def generate_cache(host, params):
    keylist = params.keys()
    for key in keylist:
        ...
        while page <= pages_num:
            for _lot in result['content']:
                ...
                if vehicle_info_item is not None:
                    ...
                    if len(Foregoing.objects.filter(parent_lot=lot['ln'])) > 0:
                        vehicle_sold_forgoing = VehicleSold.objects.filter(info__vin=vin).order_by('-info__sale_date')[0]
                        if (len(vehicle_sold_forgoing) != 0):
                            Foregoing.objects.filter(parent_lot=lot['ln'], foregoing_lot=vehicle_sold_forgoing['info__lot']).delete()
...
@start_new_thread
def generating_cache(host, params):
    keylist = params.keys()
    for key in keylist:
        params[key] = params[key][0]
    if 'status' in params:
...
def scrap_not_exist_lots():
    ...
    for notexist_lot in notexist_lots:
        ...
        if len(Foregoing.objects.filter(parent_lot=lot['ln'])) > 0:
            vehicle_sold_forgoing = VehicleSold.objects.filter(info__vin=vin).order_by('-info__sale_date')[0]
            if (len(vehicle_sold_forgoing) != 0):
                Foregoing.objects.filter(parent_lot=lot['ln'],
                                         foregoing_lot=vehicle_sold_forgoing['info__lot']).delete()
